...
def parse_args():
    ...
    parser.add_argument("--train-seed", type=int, default=0, help="Model seed index from training to load. Default = 0.")
...
if __name__ == "__main__":
    ...
    if args.env not in ["particle", "astrobee", "quadrotor"]:
        ...
        baseline_checkpoint_folder_path = "./checkpoints/particle_baseline"
        ...
        baseline_env = PointParticleLissajousTracking(equivariant=baseline_train_config["EQUIVARIANT"], terminate_on_error=baseline_train_config["TERMINATE_ON_ERROR"], 
                                            reward_q_pos=baseline_train_config["REWARD_Q_POS"], reward_q_vel=baseline_train_config["REWARD_Q_VEL"], 
                                            ...
                                            use_des_action_in_reward=baseline_train_config["USE_DES_ACTION_IN_REWARD"], reward_fn_type=baseline_train_config["REWARD_FN_TYPE"],)
        
        equivariant_checkpoint_folder_path = "./checkpoints/particle_pva_equivariant"
        ...
        equivariant_env = PointParticleLissajousTracking(equivariant=equivariant_train_config["EQUIVARIANT"], terminate_on_error=equivariant_train_config["TERMINATE_ON_ERROR"], 
                                            reward_q_pos=equivariant_train_config["REWARD_Q_POS"], reward_q_vel=equivariant_train_config["REWARD_Q_VEL"], 
                                            ...
                                            use_des_action_in_reward=equivariant_train_config["USE_DES_ACTION_IN_REWARD"], reward_fn_type=baseline_train_config["REWARD_FN_TYPE"],)
        ...
        baseline_checkpoint_folder_path = "./checkpoints/quadrotor_baseline"
        baseline_load_path = baseline_checkpoint_folder_path + "/model_final/"
        baseline_load_path = os.path.abspath(baseline_load_path)

        baseline_train_config = ast.literal_eval(open(baseline_checkpoint_folder_path+"/config.txt", "r").read())
        baseline_env = SE2xRQuadLissajous(equivariant=baseline_train_config["EQUIVARIANT"], terminate_on_error=baseline_train_config["TERMINATE_ON_ERROR"], 
                                            reward_q_pos=baseline_train_config["REWARD_Q_POS"], reward_q_vel=baseline_train_config["REWARD_Q_VEL"], reward_q_rotm=baseline_train_config["REWARD_Q_ROTM"], reward_q_omega=baseline_train_config["REWARD_Q_OMEGA"], 
                                            reward_r=baseline_train_config["REWARD_R"], reward_reach=baseline_train_config["REWARD_REACH"], 
                                            termination_bound=baseline_train_config["TERMINATION_BOUND"], terminal_reward=baseline_train_config["TERMINAL_REWARD"], 
                                            state_cov_scalar=baseline_train_config["STATE_COV_SCALAR"], ref_cov_scalar=baseline_train_config["REF_COV_SCALAR"], 
                                            use_des_action_in_reward=baseline_train_config["USE_DES_ACTION_IN_REWARD"], use_abs_reward_fn=baseline_train_config["USE_ABS_REWARD_FN"],)
        
        equivariant_checkpoint_folder_path = "./checkpoints/quadrotor_equivariant"
        equivariant_load_path = equivariant_checkpoint_folder_path + "/model_final/"
        equivariant_load_path = os.path.abspath(equivariant_load_path)

        equivariant_train_config = ast.literal_eval(open(equivariant_checkpoint_folder_path+"/config.txt", "r").read())
        equivariant_env = SE2xRQuadLissajous(equivariant=equivariant_train_config["EQUIVARIANT"], terminate_on_error=equivariant_train_config["TERMINATE_ON_ERROR"], 
                                            reward_q_pos=equivariant_train_config["REWARD_Q_POS"], reward_q_vel=equivariant_train_config["REWARD_Q_VEL"], reward_q_rotm=equivariant_train_config["REWARD_Q_ROTM"], reward_q_omega=equivariant_train_config["REWARD_Q_OMEGA"], 
                                            reward_r=equivariant_train_config["REWARD_R"], reward_reach=equivariant_train_config["REWARD_REACH"], 
                                            termination_bound=equivariant_train_config["TERMINATION_BOUND"], terminal_reward=equivariant_train_config["TERMINAL_REWARD"], 
                                            state_cov_scalar=equivariant_train_config["STATE_COV_SCALAR"], ref_cov_scalar=equivariant_train_config["REF_COV_SCALAR"], 
                                            use_des_action_in_reward=equivariant_train_config["USE_DES_ACTION_IN_REWARD"], use_abs_reward_fn=equivariant_train_config["USE_ABS_REWARD_FN"],)


    # Load model
    # Define the model
    if args.env == "particle":
        baseline_model = ActorCritic(action_dim=3, activation=baseline_train_config.get("ACTIVATION", "tanh"), num_layers=baseline_train_config.get("NUM_LAYERS", 3), num_nodes=baseline_train_config.get("NUM_NODES", 64))
        ...
        equivariant_model = ActorCritic(action_dim=3, activation=equivariant_train_config.get("ACTIVATION", "tanh"), num_layers=equivariant_train_config.get("NUM_LAYERS", 3), num_nodes=equivariant_train_config.get("NUM_NODES", 64))
        ...
        baseline_model = ActorCritic(action_dim=4, activation=baseline_train_config.get("ACTIVATION", "tanh"), num_layers=baseline_train_config.get("NUM_LAYERS", 3), num_nodes=baseline_train_config.get("NUM_NODES", 64))
        baseline_model.init(rng, jnp.zeros(baseline_env.observation_space().shape))
        # save_path_base = os.path.dirname(load_path)
        print("Loading model from: ", baseline_load_path)
        baseline_model_params = orbax_cp.PyTreeCheckpointer().restore(baseline_load_path)[0]['params']['params']
        ...
        equivariant_model = ActorCritic(action_dim=4, activation=equivariant_train_config.get("ACTIVATION", "tanh"), num_layers=equivariant_train_config.get("NUM_LAYERS", 3), num_nodes=equivariant_train_config.get("NUM_NODES", 64))
        equivariant_model.init(rng, jnp.zeros(equivariant_env.observation_space().shape))
        print("Loading model from: ", equivariant_load_path)
        equivariant_model_params = orbax_cp.PyTreeCheckpointer().restore(equivariant_load_path)[0]['params']['params']
        equivariant_model_params = select_seed_params(equivariant_model_params, args.train_seed)
    ...
    if args.env == "particle":
        save_path = "./ros2_ws/src/trajectory_visualization/trajectory_visualization/data/particle/"
        ...
        save_path = "./ros2_ws/src/trajectory_visualization/trajectory_visualization/data/quadrotor/"
        np.save(save_path+"baseline_pos.npy", np.asarray(baseline_pos))
        np.save(save_path+"baseline_rotm.npy", np.asarray(baseline_rotm))
        np.save(save_path+"equivariant_pos.npy", np.asarray(equivariant_pos))
        np.save(save_path+"equivariant_rotm.npy", np.asarray(equivariant_rotm))
